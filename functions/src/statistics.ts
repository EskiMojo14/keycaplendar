...
const filterPropSets = (
  ...
  sets.filter(
    (set) =>
      (prop === "vendor" &&
        set.vendors &&
        set.vendors.findIndex((vendor) => vendor.name === val) !== -1) ||
...
const sanitiseBarData = (data: Record<string, number>[]) =>
  data
    .map((datum, index, array) =>
      array.length > 1 ? { ...datum, index } : datum
    )
    .filter(
      (datum, i, array) =>
        Object.keys(datum).length > (array.length > 1 ? 1 : 0)
    );

const createTimelinesData = (
  icSets: StatisticsSetType[],
  gbSets: StatisticsSetType[]
) => {
  ...
  objectKeys(timelinesData).forEach((cat) => {
    ...
    const profileNames = alphabeticalSort(
      removeDuplicates(catSets.map((set) => set.profile))
    );
    const designerNames = alphabeticalSort(
      removeDuplicates(catSets.map((set) => set.designer).flat(1))
    );
    ...
    const profileMonths = months.map((month) => {
      ...
      return profileNames.reduce<TimelinesDataObject<true>["months"][number]>(
        (obj, profile) => {
          ...
          return profileSets.length > 0
            ? { ...obj, [profile]: profileSets.length }
            : obj;
        },
        monthSets.length > 0
          ? {
    ...
    timelinesData[cat].breakdown = objectKeys(
      ...
      const data = list.map((name) => {
        ...
        if (property === "profile") {
          ...
          const profiles = alphabeticalSort(
            removeDuplicates(filteredSets.map((set) => set.profile))
          );
          const monthsData = months.map((month) => {
            ...
            return profiles.reduce<TimelinesDataObject<true>["months"][number]>(
              (obj, profile) => {
                const profileSets = filterPropSets(
                  monthSets,
                  "profile",
                  profile
                );
      ...
      data.sort(
        (a, b) =>
          alphabeticalSortPropCurried("total", true)(a, b) ||
...
const createCalendarData = (
  ...
  objectKeys(calendarData).forEach((cat) => {
    ...
    const sortedDates = removeDuplicates(
      alphabeticalSort(catSets.map((set) => set[cat]))
    );
    const [startDate] = sortedDates;
    ...
    const createCalendarDataObject = (
      sets: StatisticsSetType[],
      name: string
    ): CalendarDataObject<true> => {
      const dayData = days
        .map<CalendarDatum<true>>((day, index) => {
          const value = sets.filter((set) =>
            cat === "gbLaunch"
              ? pageConditions(set, day).live
              : set.icDate === day
          );
          return {
            index,
    ...
    calendarData[cat].summary = createCalendarDataObject(
      catSets,
      calendarData[cat].summary.name
    );

    calendarData[cat].breakdown = objectKeys(
      calendarData[cat].breakdown
    ).reduce((obj, prop) => {
      const list = lists[prop];
      return {
        ...obj,
        [prop]: list.map((name) =>
          createCalendarDataObject(filterPropSets(catSets, prop, name), name)
        ),
      };
  ...
  data.map(({ sunburst, pie = {}, ...datum }) => {
    const filteredPie = objectEntries(pie)
      ...
      .reduce(
        ...
        {} as StatusDataObject<true>["pie"]
      );
    const filteredSunburst = sunburst
      ? sunburst
          .map((child, index, array) =>
            array.length > 1 ? { ...child, index } : child
          )
          .map((child) =>
            sunburstChildHasChildren(child)
              ? {
                  ...
                  children: child.children.filter((child) =>
                    sunburstChildHasChildren(child) ? true : child.val > 0
                  ),
                }
              : child
          )
...
const createStatusData = (sets: StatisticsSetType[]) => {
  ...
  const designerNames = alphabeticalSort(
    removeDuplicates(sets.map((set) => set.designer).flat(1))
  );
  const vendorNames = alphabeticalSort(
    removeDuplicates(
  ...
  const createStatusDataObject = (
    ...
    const icSets = objSets.filter(
      (set: StatisticsSetType) => !set.gbLaunch || set.gbLaunch.includes("Q")
    );
    ...
    const liveGbSets = objSets.filter((set) => {
      ...
      return (
        startDate && startDate <= today && (endDate >= yesterday || !set.gbEnd)
      );
    });
    ...
    if (prop && list) {
      ...
      const sunburst: StatusDataObjectSunburstChild<true>[] = sunburstSets.map(
        (statusSets) => {
  ...
  [statusData.summary] = sanitiseStatusData([
    createStatusDataObject(
      sets,
      statusData.summary.name,
      "profile",
      profileNames
    ),
  ]);
...
const sanitiseShippedData = (
  data: ShippedDataObject<true>[]
): ShippedDataObject<true>[] =>
...
const createShippedData = (sets: StatisticsSetType[]) => {
  ...
  const profileNames = alphabeticalSort(
    removeDuplicates(pastSets.map((set) => set.profile))
  );
  const designerNames = alphabeticalSort(
    removeDuplicates(pastSets.map((set) => set.designer).flat(1))
  );
  const vendorNames = alphabeticalSort(
    removeDuplicates(
      pastSets
  ...
  const createShippedDataObject = (
    ...
    const data = months.map((month) => {
      const filteredShippedSets = filterCatSetsByMonth(
        shippedSets,
        "gbEnd",
        month
      );
      const filteredUnshippedSets = filterCatSetsByMonth(
        unshippedSets,
        "gbEnd",
        month
      );
      return {
        unshipped: filteredUnshippedSets.length,
        shipped: filteredShippedSets.length,
      };
    });
    return {
      name: name,
  ...
  [shippedData.summary] = sanitiseShippedData([
    createShippedDataObject(pastSets, shippedData.summary.name),
  ]);
...
const sanitiseCountData = (
  data: CountDataObject<true>[],
  idIsIndex = false
): CountDataObject<true>[] =>
  data.map(({ data, name, total, mode, range, ...datum }) => {
    ...
    const filteredData = data
      ...
      .map((datum: any, index: any, array: any[]) =>
        array.length > 1 && !idIsIndex ? { ...datum, index } : datum
      )
    ...
    return {
      ...
      range,
      mode:
        (mode && mode.length > 1) || (mode && mode[0] > 0) ? mode : undefined,
      data: filteredData,
    };
  });

const createDurationData = (
  icSets: StatisticsSetType[],
  gbSets: StatisticsSetType[]
) => {
  ...
  categories.forEach((cat) => {
    ...
    if (hasKey(durationData, cat)) {
      const profileNames = alphabeticalSort(
        removeDuplicates(propSets.map((set) => set.profile))
      );
      const designerNames = alphabeticalSort(
        removeDuplicates(propSets.map((set) => set.designer).flat(1))
      );
      const vendorNames = alphabeticalSort(
        removeDuplicates(
          propSets
      ...
      const createDurationDataObject = (
        data: number[],
        name: string,
        total: number
      ): CountDataObject<true> => {
        const labels = [
          ...math.range(0, math.round(math.max(data)), 1, true).toArray(),
        ] as number[];
      ...
      const summaryData = propSets.map((set) => {
        ...
        const endDate = DateTime.fromISO(
          set[cat === "gbLaunch" ? "gbEnd" : "gbLaunch"],
          { zone: "utc" }
        );
        const length = endDate.diff(
          startDate,
          cat === "icDate" ? "months" : "days"
        );
      ...
      [durationData[cat].summary] = sanitiseCountData(
        [
          createDurationDataObject(
            summaryData,
            durationData[cat].summary.name,
            propSets.length
          ),
        ],
        true
...
const createVendorsData = (sets: StatisticsSetType[]) => {
  ...
  const profileNames = alphabeticalSort(
    removeDuplicates(vendorSets.map((set) => set.profile))
  );
  const designerNames = alphabeticalSort(
    removeDuplicates(vendorSets.map((set) => set.designer).flat(1))
  );
  const vendorNames = alphabeticalSort(
    removeDuplicates(
      vendorSets
  ...
  const createVendorsDataObject = (
    data: number[],
    name: string,
    total: number
  ): CountDataObject<true> => {
    const labels = [...math.range(0, math.max(data), 1, true).toArray()].flat();
    const range = math.max(data) - math.min(data);
    const rangeDisplay = `${math.min(data)} - ${math.max(data)} (${range})`;
    return {
      name: name,
  ...
  [vendorsData.summary] = sanitiseCountData(
    [
      createVendorsDataObject(
        summaryLengthArray,
        vendorsData.summary.name,
        vendorSets.length
      ),
    ],
    true
...
export const createStatistics = functions
  ...
  .https.onCall(async (data, contextverylongnameevenlongerpls) => {
    ...
    const sets: StatisticsSetType[] = snapshot.docs
      .map((doc) => {
        const { profile, colorway, designer, icDate, gbEnd, shipped, vendors } =
          doc.data();

        const lastOfMonth = DateTime.fromISO(doc.data().gbLaunch, {
          zone: "utc",
        }).daysInMonth;
        const gbLaunch =
