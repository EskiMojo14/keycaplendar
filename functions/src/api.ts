...
export const getAllKeysets = functions.https.onRequest(
  async (request, response) => {
    const auth = verify(request);
    if (auth === false) {
      response.status(401).send({ error: "Unauthorized" });
    }
    const returnKeysets = async (
    ...
    ): dateFilter is typeof dateSorts[number] => {
      return Boolean(dateFilter) && arrayIncludes(dateSorts, dateFilterQuery);
    };
    ...
    if (
      validDateFilter(dateFilterQuery) &&
      validDate(beforeDateQuery) &&
      validDate(afterDateQuery)
    ) {
      ...
      const filteredRef = keysetsRef.where(
        dateFilterQuery,
        "<=",
        beforeDateQuery
      );
...
export const getKeysetsByPage = functions.https.onRequest(
  async (request, response) => {
    const auth = verify(request);
    if (auth === false) {
      response.status(401).send({ error: "Unauthorized" });
...
export const getKeysetById = functions.https.onRequest(
  async (request, response) => {
    const auth = verify(request);
    if (auth === false) {
      response.status(401).json({ error: "Unauthorized" });
