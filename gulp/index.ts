...
const { p: format } = yargs(process.argv.slice(2))
  .options({
    p: { alias: ["format", "prettier"], default: false, type: "boolean" },
  })
  .parse();

const objectEditDeep =
  <InputVal = string, OutputVal = string>(
    keyCB: (string: string) => string = (string) => string,
    valCB: (V: InputVal) => OutputVal = (val) => val as unknown as OutputVal
  ) =>
  <
    T extends Record<string, InputVal | OutputVal>,
    Nested extends Record<string, T> | T | T[]
  >(
    input: Nested
  ): Nested => {
    if (typeof input !== "object") return input;
    if (Array.isArray(input))
      return input.map(objectEditDeep(keyCB, valCB)) as Nested;
...
const handleLists = (object: any): any =>
  Object.entries(object).reduce<any>((prev, [mapName, mapVal]) => {
    ...
    if (typeof copy[map] === "object") {
      if (typeof mapVal === "string" || mapVal instanceof Array) {
        ...
        Object.entries(mapVal as Record<string, unknown>).forEach(
          ([keyIndex, val]) => {
            if (typeof copy[map][theme] === "object") {
              if (keyIndexRegex.test(keyIndex)) {
                const [_full, key, index] = keyIndex.match(
                  keyIndexRegex
                ) as string[];
...
export const exportSCSS = (cb: () => void) => {
  const options: sass.LegacyOptions<"sync"> = {
    file: "src/export.scss",
    includePaths: ["src/", "node_modules/"],
  };
  const result = sass.renderSync(options);
  const css = result.css.toString();
  const jsonString = cssToJS(css).replace(
    /(['"])?([a-zA-Z0-9_]+)(['"])?:/g,
    '"$2": '
  );
  ...
  const editKey = (key: string) =>
    key
      .replace(/Element$/, "")
      .replace(
        /(graphColors|themesMap)(.*)/,
        (_full, group, name) => `${group}__${camelCase(name)}`
      )
      .replace(
        /ARRAY(.*)INDEX/g,
        (_full, name) => `ARRAY${camelCase(name)}INDEX`
      )
  ...
  if (format) {
    prettier
      ...
      .then((options) => {
        if (options) {
          writeFile(
            prettier.format(stringify(listProcessed), {
              ...options,
              parser: "json",
            })
          );
