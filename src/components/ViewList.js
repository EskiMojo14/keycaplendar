        ...
        return (
            <List twoLine className="view-list">
                {this.props.sets.map((set, index) => {
                    ...
                    return (
                        <ElementList page={this.props.page} selected={(this.props.detailSet === set || this.props.editSet === set)} set={set} title={title} subtitle={subtitle} image={set.image} details={this.props.details} closeDetails={this.props.closeDetails} store={set.storeLink} admin={this.props.admin} edit={this.props.edit} thisWeek={thisWeek} live={live} key={index} />
                    )
                })}
                <ListDivider />
            </List>
