...
export class ElementCard extends React.Component {
    render() {
        const liveIndicator = (this.props.live && this.props.page !== 'live' ? (
        ...
        const timeIndicator = (this.props.thisWeek ? (<Typography use="overline" tag="h4" className="time-indicator">{this.props.daysLeft} day{(this.props.daysLeft > 1 ? 's' : '')}</Typography>) : '');
        return (
