...
export class ElementList extends React.Component {
    render() {
        const liveIndicator = (this.props.live && this.props.page !== 'live' ? (<ListItemMeta className="live-indicator" icon={{
            strategy: 'component',
            icon: (
        ...
        const timeIndicator = (this.props.thisWeek ? (<Typography use="overline" className="time-indicator">{this.props.daysLeft} day{(this.props.daysLeft > 1 ? 's' : '')}</Typography>) : '');
        return (
