...
describe("removeDuplicates", () => {
  it("removes duplicates from an array", () => {
    expect(removeDuplicates(["test", "test", "test2"])).toEqual(["test", "test2"]);
  });
});
...
describe("alphabeticalSort", () => {
  describe("alphabeticalSortCurried", () => {
    it("provides a sort function that sorts strings alphabetically", () => {
      expect(["a", "c", "b"].sort(alphabeticalSortCurried())).toEqual(["a", "b", "c"]);
    });
    it("sorts in descending order if specified", () => {
      expect(["a", "c", "b"].sort(alphabeticalSortCurried(true))).toEqual(["c", "b", "a"]);
    });
    it("hoists a specified value if provided", () => {
      expect(["a", "c", "b"].sort(alphabeticalSortCurried(false, "c"))).toEqual(["c", "a", "b"]);
    });
  });
  describe("alphabeticalSort", () => {
  ...
  describe("alphabeticalSortPropCurried", () => {
    it("provides a sort function that sorts objects alphabetically by a specified property", () => {
      expect([{ test: "a" }, { test: "c" }, { test: "b" }].sort(alphabeticalSortPropCurried("test"))).toEqual([
        { test: "a" },
        { test: "b" },
        { test: "c" },
      ]);
    ...
    it("sorts in descending order if specified", () => {
      expect([{ test: "a" }, { test: "c" }, { test: "b" }].sort(alphabeticalSortPropCurried("test", true))).toEqual([
        { test: "c" },
        { test: "b" },
        { test: "a" },
      ]);
    ...
    it("hoists a specified value if provided", () => {
      expect(
        [{ test: "a" }, { test: "c" }, { test: "b" }].sort(alphabeticalSortPropCurried("test", false, "c"))
      ).toEqual([{ test: "c" }, { test: "a" }, { test: "b" }]);
    });
  });
  describe("alphabeticalSortProp", () => {
    it("sorts objects alphabetically by a specified property", () => {
      expect(alphabeticalSortProp([{ test: "a" }, { test: "c" }, { test: "b" }], "test")).toEqual([
        { test: "a" },
        { test: "b" },
        { test: "c" },
      ]);
    ...
    it("sorts in descending order if specified", () => {
      expect(alphabeticalSortProp([{ test: "a" }, { test: "c" }, { test: "b" }], "test", true)).toEqual([
        { test: "c" },
        { test: "b" },
        { test: "a" },
      ]);
    ...
    it("hoists a specified value if provided", () => {
      expect(alphabeticalSortProp([{ test: "a" }, { test: "c" }, { test: "b" }], "test", false, "c")).toEqual([
        { test: "c" },
        { test: "a" },
        { test: "b" },
      ]);
...
describe("pluralise", () => {
  it("pluralises correctly, using custom plural if provided", () => {
    ...
    const { cows, pigs, geese } = farm;
    expect(
      pluralise`I have ${cows} ${[cows, "cow"]}, ${pigs} ${[pigs, "pig"]}, and ${geese} ${[geese, "goose", "geese"]}`
...
describe("arrayMove", () => {
  it("moves item at specified index to new index", () => {
    expect(arrayMove(["first", "second", "third"], 1, 2)).toEqual(["first", "third", "second"]);
  });
});
